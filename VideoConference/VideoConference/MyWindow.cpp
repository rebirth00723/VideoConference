...
MyWindow::MyWindow(char* ip, short port){
	...
	*/
	...
	QObject::connect(sendBtn, SIGNAL(clicked()), line, SLOT(clear()));
	QObject::connect(line, SIGNAL(editingFinished()), this, SLOT(gettext()));
	QObject::connect(line, SIGNAL(editingFinished()), line, SLOT(clear()));
...
void MyWindow::setID(int myid)
{
	ID = myid+'0';
}
void MyWindow::setID(char* myid)
{
	ID = myid;
}